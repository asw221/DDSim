...
// Match Properties
enum KPDCrossmatch { CROSSMATCH_SUCCESSFUL,
		     CROSSMATCH_O_DONOR_TO_NON_O_CANDIDATE, 
		     CROSSMATCH_REQUIRES_DESENSITIZATION,
		     CROSSMATCH_REQUIRES_DESENSITIZATION_AND_O_TO_NON_O,
		     CROSSMATCH_FAILED };
...
namespace KPDFunctions {
  ...
  inline std::string csvStringToBlank(const std::string &str) {
    const std::regex anyButComma("[^,]+");
    const std::string blank = std::regex_replace(str, anyButComma, "");
    return blank;
  };
  ...
  // Function to sanity-check integer programming outputs  - AW
  inline bool isValidSolution(
    const std::vector<std::vector<int> > &arrangements,
    const std::vector<int> &solutionIndex
  ) {
    // Somewhat naive implementation, but should be relatively fast.
    // Consider changing if solution structure becomes more complex.
    // @param arrangements  should be a container of vectors of all the
    //    node ID's in possible arrangements
    // @param solutionIndex  should be a container of integers indexing
    //    the arrangements that correspond to a solution
    //
    // Pararameters should match input and output of
    // KPDMatchRun::getOptimalSolutionForCurrentMatchRun(), respectively;
    // all nodes in a solution should appear exactly once
    //
  
    bool solutionIsValid = true;
    std::map<int, int> nodeCounter;
  
    if (arrangements.empty() || solutionIndex.empty())
      return solutionIsValid;

    // Otherwise, build map containing counts of all the node ID's in the
    // solution and print any information about pathological solutions
    for (std::vector<int>::const_iterator it = solutionIndex.cbegin();
	 it != solutionIndex.cend(); it++) {
      for (std::vector<int>::const_iterator arrIt = arrangements[*it].cbegin();
	   arrIt != arrangements[*it].cend(); arrIt++) {
	if (nodeCounter.empty() || nodeCounter.find(*arrIt) == nodeCounter.end()) {
	  nodeCounter.insert(std::pair<int, int>(*arrIt, 1));
	}
	else {
	  nodeCounter[*arrIt]++;
    ...
    if (!solutionIsValid) {
      std::cout << "\n*** Invalid Solution *******************\n";
      for (std::map<int, int>::const_iterator it = nodeCounter.cbegin();
	   it != nodeCounter.cend(); it++) {
	if (it->second > 1) {
  ...
  // Overloaded version (for fallbacks, currently)  - AW
  inline bool isValidSolution(
    const std::vector<std::vector<int> > &arrangements
  ) {
    std::vector<int> indexAll(arrangements.size());
    for (int i = 0; i < arrangements.size(); i++)
      indexAll[i] = i;
    return isValidSolution(arrangements, indexAll);
  };
