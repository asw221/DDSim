...
private:
  ...
  std::string fileInitialKPD;
  std::string fileInitialKPDTransplants;
...
public:
  ...
  int getInitKPDSize();
  int getInitKPDPairsAddedPerMatchRun() const;
  ...
  double getProbNodeFailure(const bool &nddNode = false) const;
  double getProbEdgeFailure(const bool &nddEdge = false) const;
...
bool KPDParameters::processParameters(std::string & fileName){
  ...
  while (!iFile.eof()){
	...
	utilityScheme = KPDFunctions::stringToUtilityScheme(tokenTwo);
	// Can be any of:
	// UTILITY_TRANSPLANTS, UTILITY_FIVE_YEAR_SURVIVAL,
	// UTILITY_TEN_YEAR_SURVIVAL, UTILITY_TRANSPLANT_DIFFICULTY,
	// UTILITY_RANDOM
      }

      //Numerical Parameters
      if (tokenOne.compare("#numberofiterations") == 0) {
	numberOfIterations = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#startingiterationid") == 0) {
	startingIterationID = atoi(tokenTwo.c_str());
      }
	...
	pairArrivals = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#nddarrivals") == 0) {
	nddArrivals = atof(tokenTwo.c_str());
      }
      
      if (tokenOne.compare("#timesimulation") == 0) {
	timeSimulation = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#timebetweenmatchruns") == 0) {
	timeBetweenMatchRuns = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#timebetweenselectionandtransplantation") == 0) {
	timeBetweenSelectionAndTransplantation = atoi(tokenTwo.c_str());
      }
      
      if (tokenOne.compare("#maxcyclesize") == 0) {
	maxCycleSize = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#maxchainlength") == 0) {
	maxChainLength = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#matchutilitylowerbound") == 0) {
	matchUtilityLowerBound = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#matchutilityupperbound") == 0) {
	matchUtilityUpperBound = atof(tokenTwo.c_str());
      }

      if (tokenOne.compare("#probpairactivetoinactive") == 0) {
	probPairActiveToInactive = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#probpairinactivetoactive") == 0) {
	probPairInactiveToActive = atof(tokenTwo.c_str());
      }

      if (tokenOne.compare("#probinitkpdattrition") == 0) {
	probInitKPDAttrition = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#probpairattrition") == 0) {
	probPairAttrition = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#probbridgedonorattrition") == 0) {
	probBridgeDonorAttrition = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#probnodefailure") == 0) {
	probNodeFailure = atof(tokenTwo.c_str());
      }
      if (tokenOne.compare("#probedgefailure") == 0) {
	probEdgeFailure = atof(tokenTwo.c_str());
      }

      if (tokenOne.compare("#praeligibilitymin") == 0) {
	praEligibilityMin = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#praeligibilitymax") == 0) {
	praEligibilityMax = atoi(tokenTwo.c_str());
      }

      //Additional Options
      if (tokenOne.compare("#estimateexpectedutility") == 0) {
	estimateExpectedUtility = (tokenTwo.compare("TRUE") == 0);
	// if (tokenTwo.compare("TRUE") == 0){ estimateExpectedUtility = true; }
	// else if (tokenTwo.compare("FALSE") == 0){ estimateExpectedUtility = false; }
      }
      if (tokenOne.compare("#numberofexpectedutilityiterations") == 0) {
	numberOfExpectedUtilityIterations = atoi(tokenTwo.c_str());
      }
			
      if (tokenOne.compare("#reserveodonorsforocandidates") == 0) {
	reserveODonorsForOCandidates = (tokenTwo.compare("TRUE") == 0);
	...
	allowABBridgeDonors = (tokenTwo.compare("TRUE") == 0);
	...
	allowDesensitization = (tokenTwo.compare("TRUE") == 0);
	// if (tokenTwo.compare("TRUE") == 0) { allowDesensitization = true; }
	// else if (tokenTwo.compare("FALSE") == 0) { allowDesensitization = false; }
      }

      //Files and Folders
      if (tokenOne.compare("#filekpddata") == 0) {
	fileKPDData = tokenTwo;
      }
      if (tokenOne.compare("#filehlafrequency") == 0) {
	fileHLAFrequency = tokenTwo;
      }
      if (tokenOne.compare("#filehladictionary") == 0) {
	fileHLADictionary = tokenTwo;
      }
      if (tokenOne.compare("#filesurvivalparameters") == 0) {
	fileSurvivalParameters = tokenTwo;
      }
      if (tokenOne.compare("#filedeceaseddonors") == 0) {
	fileDeceasedDonors = tokenTwo;
      }
      if (tokenOne.compare("#filewaitinglistcandidates") == 0) {
	fileWaitingListCandidates = tokenTwo;
      }
      

      //Random Number Generator Seeds
      if (tokenOne.compare("#rngseedselection") == 0) {
	rngSeedSelection = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseedattrition") == 0) {
	rngSeedAttrition = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseedarrival") == 0) {
	rngSeedArrival = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseedmatch") == 0) {
	rngSeedMatch = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseeddonor") == 0) {
	rngSeedDonor = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseedstatus") == 0) {
	rngSeedStatus = atoi(tokenTwo.c_str());
      }
      if (tokenOne.compare("#rngseedexpectedutility") == 0) {
	rngSeedExpectedUtility = atoi(tokenTwo.c_str());
      }

      //Output Suppression
      if (tokenOne.compare("#suppressexchangeinformation") == 0) {
	suppressExchangeInformation = (tokenTwo.compare("TRUE") == 0);
	...
	suppressSimulationInformation = (tokenTwo.compare("TRUE") == 0);
	...
	suppressPopulationInformation = (tokenTwo.compare("TRUE") == 0);
...
int KPDParameters::getInitKPDPairsAddedPerMatchRun() const {
  return initKPDPairsAddedPerMatchRun;
};
...
double KPDParameters::getProbNodeFailure(const bool &nddNode) const {
  if (nddNode)
    return 0.0;
  return probNodeFailure;
};

double KPDParameters::getProbEdgeFailure(const bool &nddEdge) const {
  if (nddEdge)
    return 0.0;
  return probEdgeFailure;
};
