...
public:
  ...
  KPDDonor(
    int id, int matchingID, KPDBloodType bt, 
    KPDRelation relation, int age, bool male,
    KPDRace race, double height, double weight,
    bool cigaretteUse
  );
  
  //Constructs a deceased donor from SRTR data
  KPDDonor(
    int id, KPDBloodType bt, bool minorA, 
    int age, bool male, KPDRace race, double height, double weight,
    bool cigaretteUse, int recoveryTime, int opo, double kdpi
  );
  
  //Constructs a donor with all defined donor characteristics
  KPDDonor(
    int id, int matchingID, KPDBloodType bt, bool minorA, 
    KPDRelation relation, int age, bool male, KPDRace race,
    double height, double weight, bool cigaretteUse,
  ...
  std::string getHLAString(const std::string &_delim = ";") const;
  ...
  //Strings
  std::string donorString(const bool &header = false); // Returns a comma-separated string
...
std::string KPDDonor::getHLAString(const std::string &_delim) const {
  std::string hla;
  for (std::vector<std::string>::const_iterator it = donorHLA.cbegin();
       it != donorHLA.cend(); it++) {
    if (it == donorHLA.cbegin())
      hla += (*it);
    else
      hla += (_delim + (*it));
  }
...
bool KPDDonor::isDeceasedDonor() const {
...
std::string KPDDonor::donorString(const bool &header) {	
  ...
  if (!header) {
    if (donorDeceased) {
      ...
      donorInfo << KPDFunctions::boolToTF(donorCigaretteUse) << ",";
      donorInfo << donorRecoveryTime << ",";
      donorInfo << KPDFunctions::boolToTF(donorBothKidneysAvailable) << ",";
      donorInfo << donorOPO << ",";
      donorInfo << donorKDPI;
    }
    else {
    ...
    if (donorDeceased) {
      donorInfo << "DonorBT,DonorHLA,DonorMinorA,"
		<< "DonorAge,DonorSex,DonorRace,"
		<< "DonorHeight,DonorWeight,DonorBMI,"
		<< "DonorCigarette,DonorRecoveryTime,DonorBothKidneys,"
		<< "DonorOPO,DonorKDPI";
    }
    else {
      donorInfo << "DonorBT,DonorHLA,DonorRelationship,"
		<< "DonorAge,DonorSex,DonorRace,"
		<< "DonorHeight,DonorWeight,DonorBMI,"
		<< "DonorCigarette";
    }
...
std::string KPDDonor::donorOutput() {
  ...
  if (donorDeceased) {
    donorInfo << KPDFunctions::indent(tab)
	      << "Donor: " << donorID << std::endl;
    donorInfo << KPDFunctions::indent(tab)
	      << "Recovery Date: " << donorRecoveryTime << std::endl;
    
    if (donorBothKidneysAvailable)
      donorInfo << KPDFunctions::indent(tab)
		<< "Both Kidneys Available" << std::endl;
    else
      donorInfo << KPDFunctions::indent(tab)
		<< "One Kidney Available" << std::endl;
    
    donorInfo << KPDFunctions::indent(tab)
	      << "OPO: " << donorOPO << std::endl;
  }
  else {
    donorInfo << KPDFunctions::indent(tab)
	      << "Donor " << donorID
	      << " (Matching ID: " << donorKPDMatchingID << ")" << std::endl;
  }

  // Print blood type info
  donorInfo << KPDFunctions::indent(tab)
	    << "BT: " << KPDFunctions::bloodTypeToString(donorBT) << std::endl;
  
  if (donorMinorA)
    donorInfo << KPDFunctions::indent(tab)
	      << "Minor A2 Subtype" << std::endl;

  // Print HLA info
  donorInfo << KPDFunctions::indent(tab) << "HLA:";
  for (std::vector<std::string>::iterator
	 hla = donorHLA.begin(); hla != donorHLA.end(); hla++)
    donorInfo << " " << *hla;
  donorInfo << std::endl;
  ...
  if (!donorDeceased)
    donorInfo << KPDFunctions::indent(tab)
	      ...
	      << KPDFunctions::relationToString(donorRelation)
	      << std::endl;
    
  donorInfo << KPDFunctions::indent(tab)
	    << "Age: " << donorAge
	    << ", Sex: " << KPDFunctions::boolToMaleFemale(donorMale)
	    << ", Race: " << KPDFunctions::raceToString(donorRace)
	    << std::endl;
  
  donorInfo << KPDFunctions::indent(tab)
	    << "Height: " << donorHeight
	    << ", Weight: " << donorWeight
	    << ", BMI: " << (donorWeight / donorHeight) / donorHeight
	    << std::endl;
  
  donorInfo << KPDFunctions::indent(tab)
	    << "Cigarette Use: " << KPDFunctions::boolToYesNo(donorCigaretteUse)
	    << std::endl;
  
  donorInfo << KPDFunctions::indent(tab)
	    << "KDPI: " << donorKDPI << std::endl;
...
KPDDonor * KPDDonor::copy() {
  KPDDonor * copyDonor = new KPDDonor(
    donorID, donorKPDMatchingID, donorBT, donorMinorA,
    donorRelation, donorAge, donorMale, donorRace,
    donorHeight, donorWeight, donorCigaretteUse,
    donorDeceased, donorRecoveryTime,
    donorBothKidneysAvailable, donorOPO, donorKDPI
  );
