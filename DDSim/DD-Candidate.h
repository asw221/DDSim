...
public:
  ...
  //Strings
  std::string candidateString(const bool &header = false); // Returns a comma-separated string
...
std::string KPDCandidate::candidateString(const bool &header) {
  ...
  if (!header) {
    ...
    if (candidateWaitlist) {
      candidateInfo << "RecipBT,RecipMinorA,RecipPRA,"
		    << "RecipPRA,RecipAge,RecipSex,"
		    << "RecipRace,RecipDiabetes,RecipHeight,"
		    << "RecipWeight,RecipBMI,RecipPrevTransplant,"
		    << "RecipTOD,RecipHepC,RecipInsurance,"
		    << "RecipEPTS,RecipEPTSPriority,RecipListingTime,"
		    << "RecipOPO";
    }
    else {
      candidateInfo << "RecipBT,RecipPRA,RecipAge,"
		    << "RecipSex,RecipRace,RecipDiabetes,"
		    << "RecipHeight,RecipWeight,RecipBMI,"
		    << "RecipPrevTransplant,RecipTOD,RecipHepC,"
		    << "RecipInsurance";
    }
  }
