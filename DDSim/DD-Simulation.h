...
private:
  ...
  std::vector<KPDNodeType> kpdNodeTypes;
  // ^^ Needs to be separate! Gets updated over the course of the simulation!
  ...
  std::map<int, std::map<int, KPDMatch *> >
    deceasedDonorMatches;
  std::map<int, std::map<int, std::vector<KPDMatch*> > >
    waitlistedCandidateMatches;
  std::map<int, std::map<int, std::vector<KPDMatch*> > >
    kpdMatches;
  ...
  void updateStatus(int index, KPDStatus newState, bool waitlist);
  void updateFailedMatch(
    int donorNodeID,
    int candidateNodeID,
    int donorIndex,
    bool waitlist
  ...
  // Output  --- (i.e. "output" denotes the *.csv files)
...
KPDSimulation::KPDSimulation(KPDParameters * parameters) {	
  ...
  if (matchRunTimes.back() != timeSimulation) {
    // Final match run occurs at end of simulation, regardless of time since last match run
...
// KPDSimulation::indexOfWaitlistedCandidate(...)
...
// KPDSimulation::indexOfKPDNode(...)




int KPDSimulation::getChild(
  int lower,
  int current,
  std::vector<int> & visitedVector,
  std::vector<std::vector<bool> > & adjacency
...
void KPDSimulation::updateStatus(int index, KPDStatus newState, bool waitlist) {
  ...
  if (waitlist) {
    ...
    if (waitlistedCandidateTransplanted[index] != TRANSPLANT_YES &&
	waitlistedCandidateStatus[index] != newState) {
      
      // Output status change
      kpdSimulationLog << currentTime
		       << ": WL " << index << " "
		       << KPDFunctions::statusToString(waitlistedCandidateStatus[index])
		       << " -> " << KPDFunctions::statusToString(newState)
		       << std::endl;
  ...
  // KPD Nodes
    ...
    //Only consider nodes not already transplanted and that actually change
    // state; ignore bridge donors
    if (kpdNodeTransplanted[index] != TRANSPLANT_YES &&
	kpdNodeStatus[index] != newState &&
	kpdNodeTypes[index] != BRIDGE) {

      // Output status change
      kpdSimulationLog << currentTime << ": "
		       << kpdNodes[index]->getID() << " "
		       << KPDFunctions::statusToString(kpdNodeStatus[index])
		       << " -> " << KPDFunctions::statusToString(newState)
		       << std::endl;
	...
	//... otherwise, set the node in progress as failed in all transplants in progress
	  ...
	  for (std::deque<KPDArrangement *>::iterator it = transplantQueue.begin();
	       it != transplantQueue.end(); it++) {
	    (*it)->setFailure(index);
	  }
	}
      }
...
// KPDSimulation::updateStatus(...)





void KPDSimulation::updateFailedMatch(
  int donorNodeID,
  int candidateNodeID,
  int donorIndex,
  bool waitlist
  ...
  if (waitlist) {
    ...
    if (donorIndex <
	waitlistedCandidateMatches[donorNodeID][candidateNodeID].size()) {
      waitlistedCandidateMatches[donorNodeID][candidateNodeID]
	[donorIndex]->setAdjacency(false);
    }
    ...
    for (std::vector<KPDMatch*>::iterator
	   it = waitlistedCandidateMatches[donorNodeID][candidateNodeID].begin();
	 it != waitlistedCandidateMatches[donorNodeID][candidateNodeID].end();
	 it++) {
      if ((*it)->getAdjacency()) {
    ...
    //... if no donors associated with donor node match with candidate,
    // update adjacency matrix to false
  ...
  // KPD -> KPD
    ...
    // Update match adjacency
    if (donorIndex < kpdMatches[donorNodeID][candidateNodeID].size())
      kpdMatches[donorNodeID][candidateNodeID][donorIndex]->setAdjacency(false);

    // Check if adjacency matrix needs to be updated...
    bool noAssociatedDonors = true;

    for (std::vector<KPDMatch*>::iterator it =
	   kpdMatches[donorNodeID][candidateNodeID].begin();
	 it != kpdMatches[donorNodeID][candidateNodeID].end(); it++) {
      if ((*it)->getAdjacency()) {
	noAssociatedDonors = false;
	break;
      }
    ...
    //...if no donors associated with donor node match with candidate,
    // update adjacency matrices to false
    if (noAssociatedDonors) {
...
void KPDSimulation::findDeceasedDonorMatches() {
  ...
  for (std::deque<KPDDonor *>::iterator itDonor = deceasedDonors.begin();
       itDonor != deceasedDonors.end(); itDonor++) {
	...
	// Perform crossmatch
	KPDCrossmatch crossmatch = kpdData->performCrossmatch(
          nodeCandidate, deceasedDonor, false);
	...
	if (kpdData->allowableMatch(crossmatch)) {
	  ...
	  KPDMatch * match = kpdRecord->generateMatch(
            nodeCandidate, deceasedDonor, crossmatch, false, true);
...
// KPDSimulation::findDeceasedDonorMatches()
...
void KPDSimulation::findWaitlistedCandidateMatches() {
  ...
  for (std::vector<KPDCandidate *>::iterator
	 itCandidate = waitlistedCandidates.begin();
       itCandidate != waitlistedCandidates.end(); itCandidate++) {
	...
	if (kpdData->allowableMatch(crossmatch)) {
	  ...
	  KPDMatch * match = kpdRecord->generateMatch(
            waitlistCandidate, donor, crossmatch, true, node->isNDD());
...
int KPDSimulation::allocateDonor(KPDDonor * donor) {
  // Function is called when a deceased donor is not allocated to their
  // real-life candidate.
  // This will have to be enhanced in order to follow the allocation
  // protocols listed in the files
  ...
  for (int i = 1; i <= waitlistedCandidates.size(); i++) {
	...
	(waitlistedCandidateTransplanted[candidateIndex] == TRANSPLANT_NO)) {				
...
void KPDSimulation::runStateTransitionStage() {
  ...
  for (int i = 1; i <= (int) kpdNodeStateTransitionTimes.size(); i++) {
	...
	// updateStatus(indexOfKPDNode(nodeIndex), newState, false);  // #####
...
// KPDSimulation::runStateTransitionStage()
...
void KPDSimulation::runMatchRunStage() {	
  ...
  KPDMatchRun * matchRun = new KPDMatchRun(
    kpdParameters, currentIteration, currentTime,
    kpdNodes, kpdNodeTypes, kpdNodeStatus, kpdNodeTransplanted,
    kpdMatches);
	
  // Find all the LRSs in the current pool
  matchRun->collectCyclesAndChainsForCurrentMatchRun(matchRunArrangements);
  std::cout << "# of Cycles and Chains = " << matchRunArrangements.size()
	    << std::endl;
  ...
  if (matchRunArrangements.size() > 0) {
    ...
    matchRun->assignExpectedUtilitiesForCurrentMatchRun(
      matchRunArrangements, assignedValueOfMatchRunArrangements); 
    std::cout << "Utilities Assigned" << std::endl;

    // ###################################################################
    // double utilSum = 0.0;
    // for (std::vector<double>::iterator it(assignedValueOfMatchRunArrangements.begin());
    // 	 it != assignedValueOfMatchRunArrangements.end(); it++)
    //   utilSum += *(it);
    // std::cout << "Utility sum: " << utilSum << "\n";
    // ###################################################################

    // Select optimal set of LRSs
    std::vector<int> optimalSolution;

    matchRun->getOptimalSolutionForCurrentMatchRun(
      optimalSolution, matchRunArrangements, assignedValueOfMatchRunArrangements);
    ...
    for (int i = 1; i <= matchRunArrangements.size(); i++) {
      ...
      // Retrieve NDD Status
      bool ndd = false;
      for (std::vector<int>::iterator nodeIt = currentArrangement.begin();
	   nodeIt != currentArrangement.end(); nodeIt++) {
	int nodeID = *nodeIt;
	...
	if (kpdNodeTypes[nodeIndex] != PAIR) {
      ...
      for (std::vector<int>::iterator optimalSolutionsIt = optimalSolution.begin();
	   optimalSolutionsIt != optimalSolution.end(); optimalSolutionsIt++) {
	if (*optimalSolutionsIt == arrangementIndex) {
      ...
      // ###################################################################
      // std::cout << "Arrangement at time, " << currentTime
      // 		<< ", with utility, " << arrangementUtility
      // 		<< ", is part of optimal solution ("
      // 		<< arrangementIsPartOfOptimalSolution
      // 		<< ")\n";
      // ###################################################################
	...
	// ###################################################################
	std::cout << "** Arrangement from optimal solution at time "
		  << currentTime << "\n";
	for (std::vector<int>::iterator it = currentArrangement.begin();
	     it != currentArrangement.end(); it++)
	  std::cout << (*it) << "-";
	std::cout << "\b \n";
	// ###################################################################
	...
	for (std::vector<int>::iterator nodeIt = currentArrangement.begin();
	  ...
	  kpdNodeTransplanted[nodeIndex] = TRANSPLANT_IN_PROGRESS;
	  // Mark Nodes in Arrangement as TRANSPLANT_IN_PROGRESS
	}

	//Create New Arrangement Object and Add to Those In Progress
	int transplantationTime = currentTime +
	  kpdParameters->getTimeBetweenSelectionAndTransplantation();

	KPDArrangement * newArrangement =
	  new KPDArrangement(currentIteration, currentTime,
			     transplantationTime, arrangementUtility,
			     arrangementNodes, arrangementNodeTypes,
			     kpdMatches);

	// std::cout << newArrangement->toLabel() << "\n";
	// std::cout << "Arrangement at time " << currentTime
	// 	  << ", with utility " << arrangementUtility << "\n";
	...
	kpdSimulationLog << newArrangement->toLabel() << " Queued" << std::endl;
      ...
      outputKPDExchanges << currentIteration << ","
			 << currentTime << ","
			 << kpdParameters->getTimeBetweenSelectionAndTransplantation() << ","
			 << currentArrangement.size() << ",";
			
      outputKPDExchanges << *currentArrangement.begin();
      for (std::vector<int>::iterator it = currentArrangement.begin() + 1;
	   it != currentArrangement.end(); it++) {
	outputKPDExchanges << ";" << *it;
      }

      outputKPDExchanges << ","
			 << KPDFunctions::boolToYesNo(ndd) << ","
			 << arrangementUtility << ",";
      outputKPDExchanges << KPDFunctions::boolToYesNo(arrangementIsPartOfOptimalSolution)
			 << std::endl;
...
void KPDSimulation::runDeceasedDonorAllocationStage() {
  ...
  while (deceasedDonorArrivalTimes.front() <= currentTime) {
    ...
    kpdSimulationLog << "Allocation of Deceased Donor " << deceasedDonorID
		     << " at Time " << currentTime << std::endl;


    int kidneysToWaitlist = 1;

    // Is the deceased donor organ eligible to match to KPD?
    // In other words, are both kidneys available, and is the KDPI between 20% and 35%?
    if (deceasedDonor->getBothKidneysAvailable()) {
      if (deceasedDonor->getKDPI() > 20 &&
	  deceasedDonor->getKDPI() <= 35) {
	...
	for (std::map<int, KPDMatch*>::iterator it = matchesInKPD.begin();
	     it != matchesInKPD.end(); it++) {
	  
	  int kpdNodeID = it->first;
	  int kpdNodeIndex = indexOfKPDNode(kpdNodeID);
	  KPDMatch * matchToKPD = it->second;

	  // If KPD candidate is not transplanted and is in active status and if match
	  ...
	  if (kpdNodeTransplanted[kpdNodeIndex] == TRANSPLANT_NO &&
	    ...
	    // If additional requirements for node to be involved in DD chain are
	    // satisfied
	  ...
	  }  // if (kpdNoteTransplanted[kpdNodeIndes] == TRANSPLANT_NO && ...
	}  // for (std::map<int, KPDMatch*>::iterator it = matchesInKPD.begin(); 
	...
	if (numberOfMatchedNodesInKPD > 0) {
	  ...
	  // Is there a priority candidate in the waitlist that must be transplanted
	  // first?
	  ...
	  for (std::vector<KPDCandidate *>::iterator it = waitlistedCandidates.begin();
	       it != waitlistedCandidates.end(); it++) {
		...
		waitlistedCandidateStatus[waitlistCandidateIndex] == STATUS_ACTIVE) {
	      ...
	      // Eligible candidates must be in the same DSA, have EPTS priority,
	      // and be of the same blood type as the donor
	      ...
	      if (deceasedDonorOPO == waitlistCandidateOPO &&
		  waitlistCandidateEPTSPriority &&
		  deceasedDonorBT == waitlistCandidateBT) {
		
		eligibleWaitlistCandidates.push_back(waitlistCandidateIndex);
	      }

	      KPDCrossmatch crossmatch = kpdData
		->performCrossmatch(waitlistCandidate, deceasedDonor, true);
		...
		std::vector<std::string> waitlistCandidateHLA = waitlistCandidate
		  ->getHLA();

		bool mismatch = false;
		for (std::vector<std::string>::iterator dHLAIt = deceasedDonorHLA.begin();
		     dHLAIt != deceasedDonorHLA.end(); dHLAIt++) {
		  for (std::vector<std::string>::iterator cHLAIt = waitlistCandidateHLA.begin();
		       cHLAIt != waitlistCandidateHLA.end(); cHLAIt++) {
		...
		if ((waitlistCandidatePRA >= 98 && waitlistCandidateOPO == deceasedDonorOPO) ||
		    !mismatch ||
	  ...
	  if (priorityCandidateFound) {
	    kpdSimulationLog << "Priority Candidate Found; Allocation Direct to Waitlist"
			     << std::endl;
	    kidneysToWaitlist = 2;
	  }
	  else if (eligibleWaitlistCandidates.size() == 0) {  // <-- ??? AW
	    kpdSimulationLog << "No Eligible Candidates on Waitlist for Chain"
			     << " Transplantation; Allocation Direct to Waitlist"
			     << std::endl;
	    kidneysToWaitlist = 2;
	  }
	  else {  // (!priorityCandidateFound && eligibleWaitlistCandidates.size() != 0)
	    ...
	    for (std::vector<int>::iterator wlIt = eligibleWaitlistCandidates.begin();
		 wlIt != eligibleWaitlistCandidates.end(); wlIt++) {
		...
		candidateBasePoints += 202.10;
	      ...
	      for (std::vector<int>::iterator kpdIt = matchedNodesInKPD.begin();
		   kpdIt != matchedNodesInKPD.end(); kpdIt++) {
		...
		if (waitlistedCandidateMatches[kpdNodeID][candidateID].size() > 0) {
		  ...
		  for (int k = 1; k <= numberOfDonors; k++) {
		    ...
		    if (waitlistedCandidateMatches[kpdNodeID][candidateID][donorIndex]
			->getAdjacency()) {
		      std::vector<std::string> donorHLA = kpdNode
			->getDonor(donorIndex)->getHLA();
		      std::vector<std::string> candidateHLA = candidate->getHLA();

		      int mismatches = 0;
		      for (std::vector<std::string>::iterator dHLAIt = donorHLA.begin();
			   dHLAIt != donorHLA.end(); dHLAIt++) {
			for (std::vector<std::string>::iterator cHLAIt = candidateHLA.begin();
			     cHLAIt != candidateHLA.end(); cHLAIt++) {
			  if (*dHLAIt == *cHLAIt) {
			    mismatches++;
			  }
			}
			...
			totalPoints += 4;
		      ...
		      else if (candidate->getAge() >= 11 &&
			       candidate->getAge() <= 17 &&
			       mismatches == 0) {
			totalPoints += 3;
		      }
		      else if (candidate->getAge() <= 10 &&
			       kpdNode->getDonor(donorIndex)->getKDPI() < 35) {
			totalPoints += 1;
		      }
		    ...
		    }  // if (waitlistedCandidateMatches[...][][]->getAdjacency())
		  }  // for (int k = 1; k <= numberOfDonors; k++)
		}  // if (waitlistedCandidateMatches[kpdNodeID][candidateID].size() > 0)
	      }  // for (std::vector<int>::iterator kpdIt = matchedNodesInKPD.begin(); ...
	    }  // for (std::vector<int>::iterator wlIt = eligibleWaitlistCandidates.begin(); ...
	    ...
	    if (optnScore > 0) {
	      KPDMatch * deceasedDonorToKPDMatch =
		deceasedDonorMatches[deceasedDonorID][selectedKPDNodeID];
	      
	      if (!deceasedDonorToKPDMatch->getSuccessfulMatch()) {
		kpdSimulationLog << "Deceased Donor to KPD Transplant not Viable; "
				 << "Allocation Direct to Waitlist" << std::endl;
		kidneysToWaitlist = 2;
	      }
	      else {
		KPDMatch * kpdToWaitlistMatch =
		  waitlistedCandidateMatches[selectedKPDNodeID]
		    [selectedWaitlistCandidateID][selectedDonorIndex];
		
		kpdNodeTransplanted[indexOfKPDNode(selectedKPDNodeID)] =
		  TRANSPLANT_YES;

		if (!kpdToWaitlistMatch->getSuccessfulMatch()) {
		  kpdSimulationLog << "KPD Transplant to Waitlist Refused; "
				   << "Re-Allocation of KPD Donor to Waitlist"
				   << std::endl;
		  
		  int allocatedCandidateIndex = allocateDonor(kpdNodes[selectedKPDNodeIndex]
							      ->getDonor(selectedDonorIndex));
		  
		  waitlistedCandidateTransplanted[allocatedCandidateIndex] =
		    TRANSPLANT_YES;
		}
		else {
		  kpdSimulationLog <<  "Successful Chain!" << std::endl;
		  waitlistedCandidateTransplanted[selectedWaitlistCandidateIndex] =
		    TRANSPLANT_YES;
		}
	      }  // if (!deceasedDonorToKPDMatch->getSuccessfulMatch())
	      ...
	      kpdSimulationLog << "No Possible Chains Through KPD; "
			       << "Allocation Direct to Waitlist" << std::endl;
	      kidneysToWaitlist = 2;
	    }  // if (!optnScore > 0)
	  }
	  ...
	  kpdSimulationLog << "No Compatible Pairs in KPD Pool; "
			   << "Allocation Direct to Waitlist" << std::endl;
	  kidneysToWaitlist = 2;
	}
      }
      else {
	kpdSimulationLog << "Deceased Donor not Eligible for Transplant through KPD; "
			 << "Allocation Direct to Waitlist" << std::endl;
	kidneysToWaitlist = 2;
      }  // if (deceasedDonor->getKDPI() > 20 && deceasedDonor->getKDPI() <= 35
    }  // if (deceasedDonor->getBothKidneysAvailable())
    else {
      kpdSimulationLog << "Only 1 Donor Organ Available; Allocation Direct to Waitlist"
		       << std::endl;
    }  // if (!deceasedDonor->getBothKindeysAvailable())

    
    // Remaining kidney gets allocated to deceased donor list
    // ---------------------------------------------------------------
    std::vector<int> realMatchedCandidateIDs = kpdData
      ->getActualTransplantedCandidates(deceasedDonorID);
    // Need function to get matched candidates

    for (int i = 1; i <= kidneysToWaitlist; i++) {
      bool foundWaitlistCandidate = false;

      for (std::vector<int>::iterator it = realMatchedCandidateIDs.begin();
	   it != realMatchedCandidateIDs.end(); it++) {
...
void KPDSimulation::runTransplantationStage() {
  // ###################################################################
  // std::cout << "Call: KPDSimulation::runTransplantationStage()\n";
  // ###################################################################
  
  //If there are transplant arrangements waiting...
  if (transplantQueue.size() > 0) {
    // std::cout << "Transplantation stage at time " << currentTime << "\n";
    ...
    if (transplantQueue.front()->getTransplantationTime() <= currentTime) {
      ...
      // ###################################################################
      // std::cout << "  **  Transplant time = "
      // 		<< transplantQueue.front()->getTransplantationTime()
      // 		<< "; currentTime = "
      // 		<< currentTime
      // 		<< "\n";
      // ###################################################################
      ...
      // bool arrangedTransplantsFailed = false;

      kpdSimulationLog << transplantArrangement->getTransplantationTime()
		       << ": TX - " << transplantArrangement->toLabel()
		       << std::endl << std::endl;

      //Update Matrices to Reflect Revealed Crossmatches
      for (std::vector<int>::iterator
	     itCandidateNode = nodesInProgress.begin();
	   itCandidateNode != nodesInProgress.end(); ++itCandidateNode) {
	...
	if (kpdNodeTypes[candidateNodeIndex] == PAIR) {
	  for (std::vector<int>::iterator
		 itDonorNode = nodesInProgress.begin();
	       itDonorNode != nodesInProgress.end(); ++itDonorNode) {

	    int donorNodeID = *itDonorNode;
	    int donorNodeIndex = indexOfKPDNode(donorNodeID);

	    //...iterate through all donors...
	    for (int k = 1; k <= kpdNodes[donorNodeIndex]->getNumberOfDonors();
		 k++) {
	      int donorIndex = k - 1;	      
	      //Only consider relevant exchanges within the transplant arrangement
	      if (transplantArrangement
		  ->getAdjacency(donorNodeID, candidateNodeID, donorIndex)) {
		...
		if (kpdMatches[donorNodeID][candidateNodeID][donorIndex]
		    ->getSuccessfulMatch()) {
		  kpdSimulationLog << "Match " << donorNodeID << "[" << k
				   << "] -> " << candidateNodeID
				   << " Successful (-> 1)" << std::endl;
		  
		  if (kpdMatches[donorNodeID][candidateNodeID][donorIndex]
		      ->getAssumedSuccessProbability() != 1.0) {
		    
		    kpdMatches[donorNodeID][candidateNodeID][donorIndex]
		      ->setAssumedSuccessProbability(1.0);
		    // Probability updated to reflect successful crossmatch
		  }
		}
		else {
		  kpdSimulationLog << "Match " << donorNodeID << "[" << k
				   << "] -> " << candidateNodeID
				   << " Failed (Removed)" << std::endl;
		  
		  updateFailedMatch(donorNodeID, candidateNodeID, donorIndex, false);
		  // arrangedTransplantsFailed = true;
		  // Remove edge from KPD graph to reflect unsuccessful crossmatch
		}
	      }  // if (transplantArrangement->getAdjacency(...
	    }  // for (int k = 1; k <= kpdNodes[donorNodeIndex]->getNumberOfDonors() ...
	  }  // for (std::vector<int>::iterator itDonorNode; ...
	}  // if (kpdNodeTypes[candidateNodeIndex] == PAIR) 
      }  // for (std::vector<int>::iterator itCandidateNode; ...
      
      // ###################################################################
      // std::cout << kpdSimulationLog.str() << "\n";
      // ###################################################################
      ...
      transplantArrangement->getOptimalSolutionForFallBackOptions(
        options,
      	kpdParameters->getMaxCycleSize(),
      	kpdParameters->getMaxChainLength(),
      	kpdParameters->getAllowABBridgeDonors(),
      	kpdParameters->getUtilityScheme()
      );
	...
	// ###################################################################
	// print options
	// for (std::vector<std::vector<int> >::iterator opIt = options.begin();
	//      opIt != options.end(); opIt++) {
	//   for (std::vector<int>::iterator innerIt = (*opIt).begin();
	//        innerIt != (*opIt).end(); innerIt++) {
	//     std::cout << *innerIt << "-";
	//   }
	//   std::cout << "\b \n";
	// }
	...
	// for (int i = 0; i < kpdNodeTypes.size(); i++)
	//   std::cout << "Node " << (i + 1) << " - "
	// 	    << KPDFunctions::nodeTypeToString(kpdNodeTypes[i])
	// 	    << "\n";
	// ###################################################################

	//Iterate through options:
	for (std::vector<std::vector<int> >::iterator
	       itOptions = options.begin();
	     itOptions != options.end(); itOptions++) {

	  std::vector<int> currentOption = *itOptions;

	  bool chain = kpdNodeTypes
	    [indexOfKPDNode(*currentOption.begin())] != PAIR;
	  ...
	  if (chain) {
	    ...
	    for (std::vector<int>::iterator
		   itNodes = currentOption.begin();
		 itNodes != currentOption.end() - 1; ++itNodes) {

	      int donorNodeID = *itNodes;
	      int candidateNodeID = *(itNodes + 1);

	      // ###################################################################
	      // std::cout << "DEBUG CHECK: " << donorNodeID << " --> " << candidateNodeID
	      // 		<< "\n";
	      // ###################################################################

	      int selectedDonor = transplantArrangement->selectDonor(
                donorNodeID, candidateNodeID,
		kpdParameters->getUtilityScheme(), true); // Select Donor

	      
	    // ###################################################################
	    // std::cout << "kpdNodeTransplanted.size() = " << kpdNodeTransplanted.size()
	    // 	      << "\ndonor node index: " << indexOfKPDNode(donorNodeID) << "\n";
	    // ###################################################################
	      
	      kpdNodeTransplanted[indexOfKPDNode(donorNodeID)] =
		TRANSPLANT_YES;
	      //  Transplantation Status of Donor Node


	      kpdSimulationLog << "TX: " << donorNodeID << "["
			       << selectedDonor << "] -> " << candidateNodeID
			       << " Recorded" << std::endl; // Output
	    }
		...
		if (kpdAdjacencyMatrix[i][bridgeNodeIndex + 1]) {
		  for (int k = 1; k <= kpdNodes[nodeIndex]->getNumberOfDonors(); k++) {
		    int donorIndex = k - 1;
		    updateFailedMatch(kpdNodes[nodeIndex]->getID(), bridgeNodeID,
				      donorIndex, false);
		  }
		}
		...
		if (kpdAdjacencyMatrix[bridgeNodeIndex + 1][i] &&
		    kpdNodeTypes[nodeIndex] != PAIR) {
		  for (int k = 1;
		       k <= kpdNodes[bridgeNodeIndex]->getNumberOfDonors();
		       k++) { /// CHECK THIS AGAIN!

		    int donorIndex = k - 1;
		    updateFailedMatch(
		      bridgeNodeID, kpdNodes[nodeIndex]->getID(),
		      donorIndex, false);
		  }
		}
	    ...
	    kpdSimulationLog << "Donors " << bridgeNodeID
			     << " now BD, returned to pool" << std::endl;
	  }
	  ...
	  //Realized Cycle
	    ...
	    for (std::vector<int >::iterator
		 ...
		 itNodes != currentOption.end() - 1; ++itNodes) {
	      ...
	      int selectedDonor = transplantArrangement
		->selectDonor(donorNodeID, candidateNodeID,
			      kpdParameters->getUtilityScheme(), true); // Select Donor

	      kpdNodeTransplanted[indexOfKPDNode(candidateNodeID)] =
		TRANSPLANT_YES; // Update Transplantation Status of Candidate Node

	      kpdSimulationLog << "Transplant " << donorNodeID << "["
			       << selectedDonor << "] -> " << candidateNodeID
			       << " Recorded" << std::endl;  //Output
	    }
	    ...
	    int selectedDonor = transplantArrangement
	      ->selectDonor(donorNodeID, candidateNodeID,
			    kpdParameters->getUtilityScheme(), true); // Final Transplant
      ...
      for (std::vector<int>::iterator
	     it = nodesInProgress.begin();
	   it != nodesInProgress.end(); it++) {
	...
	if (kpdNodeTransplanted[nodeIndex] == TRANSPLANT_IN_PROGRESS) {
	  ...
	  kpdSimulationLog
	    << KPDFunctions::nodeTypeToString(kpdNodeTypes[nodeIndex])
	    << " " << *it << " returned ";					
	}
      }
      ...
      // ###################################################################
      // std::cout << kpdSimulationLog.str() << std::endl << std::endl;
      // ###################################################################
...
void KPDSimulation::runIteration(int iteration) {
  ...
  deceasedDonors =
    kpdData->getDeceasedDonors();
  deceasedDonorArrivalTimes =
    kpdData->getDeceasedDonorArrivalTimes();

  // ###################################################################
  // printDeceasedDonors();
  // std::cout << "First 100 DD Arrival times:\n";
  // for (int i = 0; i < 100; i++)
  //   std::cout << deceasedDonorArrivalTimes[i] << ", ";
  // std::cout << "\b\b  \n";
  // ###################################################################
  ...
  waitlistedCandidateStateTransitions =
    kpdData->getWaitlistCandidatesStateTransitionMatrix();
  waitlistedCandidateStateTransitionTimes =
    kpdData->getWaitlistCandidatesStateTransitionTimeMatrix();
  ...
  kpdNodeStateTransitions =
    kpdRecord->getKPDNodeStateTransitionMatrix();
  kpdNodeStateTransitionTimes =
    kpdRecord->getKPDNodeStateTransitionTimeMatrix();
  ...
  kpdToWaitlistAdjacencyMatrix.assign(
    1 + (int)kpdNodes.size(),
    std::vector<bool>(1 + (int)waitlistedCandidates.size(), false)); // Blank matrix
...
void KPDSimulation::printLog() {
  std::string logFile = "output/" + kpdParameters->getOutputFolder() +
    "/" + kpdParameters->getSubFolder() + "/Log-Simulation.txt";
...
// ###################################################################
// void KPDSimulation::printDeceasedDonors() {
//   const std::string outputFilePath = "_test/_DeceasedDonorsLog.txt";
//   const std::string verticalDelim =
//     "\n----------------------------------------------------------------------\n\n";
//   std::ofstream outputFile(outputFilePath);
//
//   outputFile << deceasedDonors.size() << " deceased donors:\n\n";
//
//   //   std::deque<KPDDonor *> deceasedDonors;
//   for (std::deque<KPDDonor *>::iterator it = deceasedDonors.begin();
//        it != deceasedDonors.end(); it++) {
//     outputFile << verticalDelim
// 	       << (*it)->donorOutput()
// 	       << verticalDelim;
//   }
//   outputFile.close();
