...
private:
  ...
  void readDataFromFile(
    std::vector<std::vector<std::vector<std::string> > > & parsedData,
    std::string fileName // Reads and parses data from files
  );
...
KPDData::KPDData(KPDParameters * params) {
  ...
  // Read data 
  readDataFromFile(hlaFrequencyData, dataFilePath + params->getFileHLAFrequency());
  readDataFromFile(hlaDictionaryData, dataFilePath + params->getFileHLADictionary());
  readDataFromFile(characteristicsData, dataFilePath + params->getFileSurvivalParameters());

  readDataFromFile(kpdData, dataFilePath + params->getFileKPDData());
  readDataFromFile(deceasedDonorData, dataFilePath + params->getFileDeceasedDonors());
  readDataFromFile(waitlistCandidatesData, dataFilePath + params->getFileWaitingListCandidates());

  kpdDataLog << std::endl;

  // ###################################################################
  std::cout << kpdDataLog.str() << "\n";
  // ###################################################################
...
void KPDData::readDataFromFile(
  std::vector<std::vector<std::vector<std::string> > > & parsedData,
  std::string fileName
) {
  std::stringstream log;
  std::ifstream iFile;
  // std::cout << "Working directory: " << getwd() << "\n";

  iFile.open(fileName);
  if (iFile.fail()) {
    // std::cerr << "Cannot open file for reading: " << getwd() + "/" + fileName << std::endl;
    std::cout << "Failure opening:\n\t" << fileName << std::endl;
  ...
  // std::cout << kpdDataLog.str() << std::endl; 
}
...
void KPDData::formDonorHLAFrequency() {
  ...
  for (std::vector<std::vector<std::vector<std::string> > >::iterator
	 hlaRow = hlaFrequencyData.begin();
       hlaRow != hlaFrequencyData.end(); hlaRow++) {
    
    std::vector<std::string> donorProfile;

    for (std::vector<std::vector<std::string> >::iterator
	   hla = hlaRow->begin();
	 hla != hlaRow->begin() + 8; hla++) {
...
void KPDData::formHLADictionary() {
  ...
  for (std::vector<std::vector<std::vector<std::string> > >::iterator
	 dictionaryRow = hlaDictionaryData.begin();
       dictionaryRow != hlaDictionaryData.end(); dictionaryRow++) {
    
    std::string antigen = (*dictionaryRow)[0][0];
    kpdDataLog << antigen << ": ";

    for (std::vector<std::string>::iterator hla = (*dictionaryRow)[1].begin();
	 hla != (*dictionaryRow)[1].end(); hla++) {
...
void KPDData::formSurvivalParameters() {
  ...
  for (std::vector<std::vector<std::vector<std::string> > >::iterator
	 paramRow = characteristicsData.begin();
       paramRow != characteristicsData.end(); paramRow++) {

    std::string characteristic = (*paramRow)[0][0];
    kpdDataLog << characteristic << ": ";

    // Store 5-year and 10-year survival parameters
    survivalParameters["5 Year " + characteristic]
      .push_back(atof((*paramRow)[2][0].c_str()));
    kpdDataLog << (*paramRow)[2][0] << " (5-Year), ";

    survivalParameters["10 Year " + characteristic]
      .push_back(atof((*paramRow)[4][0].c_str()));
    kpdDataLog << (*paramRow)[4][0] << " (10-Year)"  << std::endl;
  }
...
void KPDData::formCharacteristicsFrequency() {
  ...
  for (std::vector<double>::iterator
	 it = characteristicsFrequency["Blood Type"].begin();
       it != characteristicsFrequency["Blood Type"].end(); it++) {
  ...
  for (std::vector<double>::iterator
	 it = characteristicsFrequency["Relation To Donor"].begin();
       it != characteristicsFrequency["Relation To Donor"].end(); it++) {
  ...
  for (std::vector<double>::iterator
	 it = characteristicsFrequency["Donor HLA BW"].begin();
       it != characteristicsFrequency["Donor HLA BW"].end(); it++) {
...
void KPDData::formKPDPopulation() {
  ...
  while (it != kpdData.end()) {
    ...
    if (ndd.compare("TRUE") == 0) {
      ...
      for (std::vector<std::string>::iterator hla = (*it)[8].begin();
	   hla != (*it)[8].end(); hla++) {
	dHLA.push_back(*hla);
      }
      ...
      KPDDonor * d = new KPDDonor(
        matchingID, donorID, dBT, dRelation, dAge, dMale,
	dRace, dHeight, dWeight, dCigaretteUse
      );

      d->setHLA(dHLA);

      kpdDataLog << "NDD " << donorID
    ...
    //Incompatible Donor-Candidate Pairing
      ...
      for (std::vector<std::string>::iterator hla = (*it)[6].begin();
	   hla != (*it)[6].end(); hla++) {
	...
	cUnacceptableHLA.push_back(*hla);
      ...
      KPDCandidate * c = new KPDCandidate(
        matchingID, candidateID, cPRA, cBT, 
	cAge, cMale, cRace, cDiabetes, cHeight,
	cWeight, cPrevTrans, cTOD, cHepC, cInsurance
      );
			
      c->setUnacceptableHLA(cUnacceptableHLA);
      c->setDesensitizableHLA(cDesensitizableHLA);
			
      kpdDataLog << "Candidate " << candidateID
		 << " (Matching ID: " << matchingID
	...
	for (std::vector<KPDDonor *>::iterator donorIt = associatedDonors.begin();
	     donorIt != associatedDonors.end(); donorIt++) {
...
void KPDData::formDeceasedDonorPopulation() {
  ...
  for (std::vector<std::vector<std::vector<std::string> > >::iterator
	 srtrRow = deceasedDonorData.begin();
       srtrRow != deceasedDonorData.end(); srtrRow++) {
    ...
    double kdpi = atoi((*srtrRow)[29][0].c_str());
...
void KPDData::formWaitlistPopulation() {
  ...
  for (std::vector<std::vector<std::vector<std::string> > >::iterator
	 srtrRow = waitlistCandidatesData.begin();
       srtrRow != waitlistCandidatesData.end(); srtrRow++) {
	...
	listingTime = atoi(listingTimeBegin.c_str());
      ...
      int age = 40;
      std::string ageCategory = (*srtrRow)[9][0];
	...
	withdrawalTime = atoi((*srtrRow)[29][0].c_str());
      ...
      bool withdrawn = (*srtrRow)[29][0].compare("NA") == 0 &&
	(*srtrRow)[30][0].compare("Removed from Waitlist") == 0;
	...
	actualDeceasedDonorWaitlistCandidateTransplants[actualDonorID].push_back(id);
      ...
      KPDCandidate * c = new KPDCandidate(
        id, pra, bt, minorA,
	age, genderMale, race, diabetes,
	height, weight,
	previousTransplant, timeOnDialysis,
	hepC, insurance, epts, eptsPriority,
  ...
  for (std::vector<int>::iterator it = waitlistCandidateIDs.begin();
       it != waitlistCandidateIDs.end(); it++) {
...
std::vector<std::deque<int> > KPDData::getWaitlistCandidatesStateTransitionTimeMatrix() {
  ...
  for (std::vector<int>::iterator it = waitlistCandidateIDs.begin();
       it != waitlistCandidateIDs.end(); it++) {
    waitlistCandidateTransitionTimes.push_back(
      waitlistCandidates[*it]->getStatusChangeTimes());
  }
...
KPDDonor * KPDData::generateDonor(std::vector<double> u) {
  // Blood Type
  int btIndex = KPDFunctions::retrieveDiscreteSampleIndex(
    u[0], characteristicsFrequency["Blood Type"]);
  ...
  for (std::vector<std::string>::iterator it = donorHLA[hlaIndex1].begin();
       it != donorHLA[hlaIndex1].end(); it++) {
    dHLA.push_back(*it);
  }

  int hlaIndex2 = KPDFunctions::retrieveDiscreteSampleIndex(u[2], donorHLAFrequency);

  for (std::vector<std::string>::iterator it = donorHLA[hlaIndex2].begin();
       it != donorHLA[hlaIndex2].end(); it++) {
    dHLA.push_back(*it);
  }
...
KPDCrossmatch KPDData::performCrossmatch(
  ...
  if ((donor->getBT() == BT_AB && candidate->getBT() != BT_AB) ||  //
      (donor->getBT() == BT_A && (candidate->getBT() == BT_O ||    //
				  candidate->getBT() == BT_B)) ||
      (donor->getBT() == BT_B && (candidate->getBT() == BT_O ||    //
				  candidate->getBT() == BT_A))) {
  ...
  if (waitlist) {
    for (std::vector<std::string>::iterator cHLA = candidateHLA.begin();
	 cHLA != candidateHLA.end(); cHLA++) {
      std::map<std::string, std::vector<std::string> >::iterator
	unacceptableAntigens = hlaDictionary.find(*cHLA);

      if (unacceptableAntigens != hlaDictionary.end()) {
	for (std::vector<std::string>::iterator
	       antigen = unacceptableAntigens->second.begin();
	     antigen != unacceptableAntigens->second.end(); antigen++) {
	  for (std::vector<std::string>::iterator dHLA = donorHLA.begin();
	       dHLA != donorHLA.end(); dHLA++) {
    ...
    for (std::vector<std::string>::iterator
	   cHLA = candidateUnacceptableHLA.begin();
	 cHLA != candidateUnacceptableHLA.end(); cHLA++) {
	...
	for (std::vector<std::string>::iterator
	  ...
	  for (std::vector<std::string>::iterator
		 dHLA = donorHLA.begin(); dHLA != donorHLA.end(); dHLA++) {
	    if ((*dHLA).compare(*antigen) == 0) {
	      return CROSSMATCH_FAILED;
	    }
	  }
    ...
    }  // for (std::vector<std::string>::iterator cHLA = ...

    for (std::vector<std::string>::iterator
	   cHLA = candidateDesensitizableHLA.begin();
	 cHLA != candidateDesensitizableHLA.end(); cHLA++) {
...
bool KPDData::allowableMatch(KPDCrossmatch crossmatch) {
  ...
  if (crossmatch == CROSSMATCH_REQUIRES_DESENSITIZATION_AND_O_TO_NON_O) {
	...
	!kpdParameters->getReserveODonorsForOCandidates()) {
      return true;
    }
  }
